...
async function updateStudentRating(studentId: number) {
...
app.post("/api/auth/signup", async (req: express.Request, res: Response) => {
  try {
...
// Update Profile
app.put("/api/auth/me", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.id;
...
app.get("/api/students", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (availability) {
      ...
      const avList = (availability as string).split(",");
...
// Smart AI Matching
app.get("/api/students/matches", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const currentUserId = req.user?.id;
    ...
    const matches = others.map(student => {
      ...
      const myWants = JSON.parse(me.skillsLearning || "[]") as string[];
      const myTeaches = JSON.parse(me.skillsTeaching || "[]") as string[];
      const theirWants = JSON.parse(student.skillsLearning || "[]") as string[];
      const theirTeaches = JSON.parse(student.skillsTeaching || "[]") as string[];
...
app.get("/api/students/:id", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
// Write Review
app.post("/api/students/:id/reviews", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const revieweeId = parseInt(req.params.id as string);
...
app.post("/api/sessions/book", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
app.patch("/api/sessions/:id/status", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
app.get("/api/messages/:studentId", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.id!;
    const contactId = parseInt(req.params.studentId as string);
...
// Send Message
app.post("/api/messages/:studentId/send", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const senderId = req.user?.id!;
    const receiverId = parseInt(req.params.studentId as string);
...
function requireAdmin(req: AuthenticatedRequest, res: Response, next: express.NextFunction) {
...
app.get("/api/admin/approvals", authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
app.post("/api/admin/approvals/:id/approve", authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const studentId = parseInt(req.params.id as string);
    await prisma.user.update({
      where: { id: studentId },
...
app.post("/api/admin/approvals/:id/reject", authenticateToken, requireAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const studentId = parseInt(req.params.id as string);
